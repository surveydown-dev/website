...
We recommend using [shinyapps.io](https://shinyapps.io/) to host your survey as it is designed to work with Shiny Apps. The platform offers a free tier that should work well for most simple surveys. However, more complex surveys may require paid plans to achieve better performance.
...
Depending on your needs, a paid plan may be worth the investment during your actual survey fielding to ensure a smooth user experience. And of course, you can consider [other hosting services](deployment.llms.md#deploying-to-other-hosting-services) that might have better options.

## Database

The surveydown package is designed to work with any PostgreSQL database, but we recommend using [Supabase](https://supabase.com/) to store your survey responses as it is a fast, reliable, and scalable PostgreSQL database. Like [shinyapps.io](https://shinyapps.io/), [Supabase](https://supabase.com/) offers a free tier that should work well for most simple surveys, but a paid plan may be needed if you expect to have a large number of concurrent respondents.
