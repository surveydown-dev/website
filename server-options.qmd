...
The `sd_skip_if()` and `sd_show_if()` functions are used to define conditional skip and show logic for the survey (see the [Conditional Control](conditional-control.qmd) page for more details), and the `sd_server()` function is used to run the survey. 

The `db = db` argument in `sd_server()` is required if you are using a database connection, which should be defined using the `sd_database()` function (see the [Store Data](store-data.qmd) page for more details). But this function also has many options arguments. This page details these options.
