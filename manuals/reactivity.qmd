...
server <- function(input, output, session) {
  ...
  # Option 2
  observe({
    output$product <- renderUI(as.numeric(input$first_number) * as.numeric(input$second_number))
  })
...
- In **option 1**, we use the `observe()` function to capture the user inputs, in which `sd_store_value()` is used to make `product` accessible in our `survey.qmd`.
- In **option 2**, upon using `observe()`, we use `output$` in front of `product`, along with `renderUI()` to make it directly accessible in `survey.qmd` without necessity of `sd_store_value()`.
