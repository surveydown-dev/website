...
If you are using RStudio, you can also make use of the page gadget to create pages under the "Addins" dropdown menu. Here is what the **Survey Page Gadget** looks like in RStudio:
...
By default, navigation buttson are automatically added to each page to allow respondents to move forward through the survey. To change the global settings (e.g., add a previous button on all pages, change the message shown on each button, etc.), you can edit the survey settings in the YAML header (see the [Survey Settings](survey-settings.html#navigation-buttons) page for details).
...
The `sd_nav()` function allows you control over the labels shown (e.g., `next_label = "Next Page"` and `prev_label = "Previous Page"`), as well as whether to show either button at all using the following arguments:
...
Finally, you can also set the target page to navigate to using the `next_page` argument (by default, the next button will send the respondent to the next page in order)

You can also conditionally control page skipping using the `sd_skip_if()` function in your server function. See the [Conditional Page Skipping](conditional-logic.html#conditional-skipping) page or [Page Navigation](page-navigation.html) page for more details.
