...
server <- function(input, output, session) {
  ...
  observeEvent(c(input$first_number, input$second_number), {
...
Since there are 2 values to be observed, we used `c(input$first_number, input$second_number)` to concatenate them. If there is only one, no concatenation is needed.
