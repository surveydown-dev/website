...
The updated documentation of the [Basic Components](https://surveydown.org/docs/basic-components) page reflects this new design.
...
The [`sd_server()`](https://pkg.surveydown.org/reference/sd_server.html) function then uses this list of page objects to display one page at a time via a [`shiny::renderUI()`](https://rdrr.io/pkg/shiny/man/renderUI.html) function into a single “main” output. This approach allowed us to control what content is being served, eliminating the ability of survey respondents to see anything other than the content on the current page.

This approach also gave us the opportunity to overhaul how pagination works in general. Previously, users had to add a `sd_next(next_page = "page_id")` button at the end of each page, making sure to specify the next page to go to. This was a bit annoying as most of the time you just want to go to the next page, so specifying it felt redundant. Now users can simply add [`sd_next()`](https://pkg.surveydown.org/reference/sd_next.html) at the bottom of each page and the server will go to the next page by default. If you want to direct the respondent to a different page, then you specify the target page using `sd_next(next_page = "page_id")`.
...
The new approach uses just two functions: [`sd_skip_if()`](https://pkg.surveydown.org/reference/sd_skip_if.html) and [`sd_show_if()`](https://pkg.surveydown.org/reference/sd_show_if.html). These functions can be provided in the main `server()` function in the `app.R` file to define the conditions and targets for either conditional displaying a question or conditionally skipping to a page. The structure for each condition in these new functions is always as follows:
...
You can provide multiple conditions to the [`sd_show_if()`](https://pkg.surveydown.org/reference/sd_show_if.html) function, each separated by a comma. The [`sd_skip_if()`](https://pkg.surveydown.org/reference/sd_skip_if.html) function works the same way, but it will skip to a target page instead of showing a target question. There is also an [`sd_stop_if()`](https://pkg.surveydown.org/reference/sd_stop_if.html) to stop the navigation under centain conditions. See the revised [Conditional Logic](https://surveydown.org/docs/conditional-logic) page for more details on the new changes.

## No more `sd_config()` function

One more small change we made is that the `sd_config()` function is no longer needed. Since we moved the conditional skip and show logic into their own functions, we took the remaining arguments that used to be provided to `sd_config()` and added them to the [`sd_server()`](https://pkg.surveydown.org/reference/sd_server.html) function as options. You can now simply pass these arguments to the [`sd_server()`](https://pkg.surveydown.org/reference/sd_server.html) function in the `app.R` file.
