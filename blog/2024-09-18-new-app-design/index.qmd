...
You can provide multiple conditions to the `sd_show_if()` function, each separated by a comma. The `sd_skip_if()` function works the same way, but it will skip to a target page instead of showing a target question. There is also an `sd_stop_if()` to stop the navigation under centain conditions. See the revised [Conditional Logic](https://surveydown.org/docs/conditional-logic) page for more details on the new changes.
