...
The free service we use for hosting a surveydown survey is [shinyapps.io](https://shinyapps.io/), which **is not HIPPA compliant**. According to the [Posit Documentation on Security and Compliance](https://docs.posit.co/shinyapps.io/guide/security_and_compliance/index.html) for [shinyapps.io](https://shinyapps.io/):
...
We designed surveydown to be able to [store data](storing-data.llms.md) on any PostgreSQL database. This gives you flexibility on where you choose to store your data, and it is up to you to find a suitable host.

Fortunately, the default service we recommend using ([Supabase](https://supabase.com/)) is actually [SOC2 Type 2 and HIPPA compliant](https://supabase.com/security) 🎉
