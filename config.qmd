...
Just as with the [`show_if_custom` argument](config.html#custom-conditional-display), we provide the `skip_if_custom` argument a list of lists, where each sublist contains a `condition` and a `target`.
