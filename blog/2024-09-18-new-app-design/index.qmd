...
You can provide multiple conditions to the `sd_show_if()` function, each separated by a comma. The `sd_skip_if()` function works the same way, but it will skip to a target page instead of showing a target question. See the revised [Configuration Options](https://surveydown.org/config-options) page for more details on the new changes.
