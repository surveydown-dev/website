...
|------|----------|--------|----------|
| `"database"` (default) | — | Red footer banner if connection fails | Live deployment; responses stored in PostgreSQL. |
...
This is distinct from `"preview"` mode: the red banner signals an unexpected failure rather than an intentional testing state. Responses will only be logged in the local `preview_data.csv` file. If you see this banner when running a live survey, check your `.env` file and credentials (see [Trouble connecting?](#trouble-connecting) below).
