...
For multiple response question types (e.g. [`mc_multiple`](question-types.html#mc_multiple)), the value returned is a vector of strings. You can use this value to check for different conditions, such as _inclusion_ using the `%in%` operator, or the _length_ using the `length()` function, like this:
...
### 4. Answering status condition

You may want to show a target question if the base question is answered. To do this, we've created an `sd_is_answered()` function to judge if a question is answered or not and returns `TRUE`/`FALSE` accordingly. For example, here is a question asking for your favorite fruit:
...
sd_question(
  type  = "mc",
  id    = "fav_fruit",
  label = "Which is your favorite fruit?",
  option = c(
    "Apple"  = "apple",
    "Banana" = "banana",
    "Orange" = "orange",
...
sd_question(
  type  = "text",
  id    = "num_fruit", 
  label = "How many do you eat per day?"
)
```

Then, say that we want to show the target question of `num_fruit` as long as the`fav_fruit` question is answered, in `server()` we can use `sd_is_answered("fav_fruit")`:
...
This way, as long as the `fav_fruit` question is answered, no matter which option the user picks, the following `num_fruit` question will appear.

::: {.callout-note}
You can also use functions like `paste0()` or `glue::glue()`, combined with `sd_output()`, to conditionally show the question label based on user's selection. For example you can write like this:

```{r}
sd_question(
  type  = "text",
  id    = "num_fruit", 
  label = glue::glue("How many {sd_output('fav_fruits', type = 'value')} do you eat in a day?")
)
```
:::

For question type of `"matrix"`, `sd_is_answered()` will be true only if all sub-questions in it are answered. For example:
...
For the above matrix question, `sd_is_answered("car_preference)` will be `TRUE` only if all questions in this matrix are answered.

## Creating custom conditions

For situations where the conditional logic is more complex, we recommend defining a custom function that will return a logical value (`TRUE` or `FALSE`) indicating whether the target question should be displayed. You can then pass this function to the `sd_show_if()` or `sd_skip_if()` functions. 

For example, let's say I had a `mc` type question where I asked how many cars the respondent owned, and I included numeric options for `1` through `5`, but then my last option was the string `"6 or more"`. Well, in this case I couldn't simple use `as.numeric(input$question_id) > 1`, because this would return `NA` if respondent chose `"6 or more"`. To address this, I could create a custom function that will handle this special condition so that it will always appropriately return `TRUE` or `FALSE`:
