...
To address this, you should always place external resources in a separate folder and then make sure that folder is added to Shiny's resource path. You can do this in your `app.R` file by using the `sd_add_folder()` function at the beginning of the file after loading the `surveydown` package, like this:

```{r}
library(surveydown)

sd_add_folder("resources")
```
