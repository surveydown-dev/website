...
```{r}
sd_show_if(
 as.numeric(sd_value("car_number")) > 1 ~ "car_ownership"
)
```
...
sd_show_if(
 ...
 all(sd_value("fav_fruits") %in% c("apple", "banana")) ~ "apple_or_banana",
...
server <- function(input, output, session) {
  ...
  more_than_one_car <- function() {
    if (is.null(sd_value("car_number"))) {
      return(FALSE)
    }
    num_cars <- as.numeric(sd_value("car_number"))
    return(num_cars > 1)
  }
